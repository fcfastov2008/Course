# Існує деяка інформація про автомобілі з кольором, роком випуску, об'ємом двигуна,
# типом автомобіля та ціною. Маємо критерії пошуку у вигляді кортежу (рік ≥, об'єм двигуна ≥, ціна ≤).
...
#def filter_cars_1(*search_criteria):
# Вводимо умови пошуку
search_criteria = (2017, 1.5,50000)
def filter_cars_1(*searches):
# Фільтрація автомобілів відповідно до умов

    filt_cars = [
        (name,deteil) for name,deteil in car_data.items()
        if  deteil[1] >= searches[0] and \
        deteil[2] >= searches[1] and \
        deteil[4] <= searches[2]]


    if not filt_cars:
        print ("За данними умовами автомобілів не знайдено")


# Сортуємо автомобілі за ціною (зростанням)

    sorted_cars = sorted(filt_cars, key=lambda x: x[1][4])




# Вивеодимо до п'яти (5) перших знайдених елементів
    #my_filters_cars = []
    for element in sorted_cars[:5]:
        print("---------------------------------------------")
        print(f"Модель: {element[0]}, Деталі: {element[1]}")


filter_cars_1(*search_criteria)