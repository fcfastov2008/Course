    ...
    if first_name is not None:
        for data in countryFile:
            if data[0] != '' and data[0] == first_name:
                ...
                break
        if country == "":
            country = choice(["India", "USA", "UK", "Canada", "Germany", "France", "Australia", "Japan"])
    else:
        filteredData = []
        for data in countryFile:
            if data[3] != '':
                filteredData.append(data[3])
        country = choice(filteredData) if filteredData else "N/A"
    return country
    ...
    try:
        for i in range(5, 12):
            ...
            if allAddrs:
                ...
                if i == 11:
                    full_addr.append(str(randint(100000, 999999)))
                else:
                    full_addr.append("N/A")
        if full_addr[5] == "N/A":
            full_addr[5] = choice(["India", "USA", "UK", "Canada", "Germany", "France", "Australia", "Japan"])
        full_addr = dict(zip(addrParam, full_addr))
    except FileNotFoundError:
        raise FileNotFoundError("data.csv not found. Ensure it is in the correct directory.")
    return full_addr
